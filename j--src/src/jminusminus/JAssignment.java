...
class JPlusAssignOp extends JAssignment {
	...
	public void codegen(CLEmitter output) {
		...
		if (lhs.type().equals(Type.STRING)) 
			...
			if		(type == Type.INT) 		output.addNoArgInstruction(IADD);
			else if (type == Type.DOUBLE)	output.addNoArgInstruction(DADD);
		}
		if (!isStatementExpression) {
...
class JMinusAssignOp extends JAssignment {
	...
	public void codegen(CLEmitter output) {
		...
		((JLhs) lhs).codegenLoadLhsRvalue(output);
		rhs.codegen(output);
		if		(type == Type.INT) 		output.addNoArgInstruction(ISUB);
		else if	(type == Type.DOUBLE) 	output.addNoArgInstruction(DSUB);
		if (!isStatementExpression) {
			// Generate code to leave the r-value atop stack
			((JLhs) lhs).codegenDuplicateRvalue(output);
		}
...
class JMultiplyAssignOp extends JAssignment {
	...
	public void codegen(CLEmitter output) {
		...
		if 		(type == Type.INT)		output.addNoArgInstruction(IMUL);
		else if	(type == Type.DOUBLE)	output.addNoArgInstruction(DMUL);
...
class JRemainderAssignOp extends JAssignment {
	...
	public void codegen(CLEmitter output) {
		...
		((JLhs) lhs).codegenLoadLhsRvalue(output);
		rhs.codegen(output);
		if		(type == Type.INT)		output.addNoArgInstruction(IREM);
		else if	(type == Type.DOUBLE)	output.addNoArgInstruction(DREM);
		if (!isStatementExpression) {
			// Generate code to leave the r-value atop stack
			((JLhs) lhs).codegenDuplicateRvalue(output);
		}
