...
class JClassDeclaration extends JAST implements JTypeDecl {
	...
	/** File type, true = class, false = interface. */
	...
	public JAST analyze(Context context) {
		...
		if(isClass)	{
			...
			// Interface -> all methods must be abstract
			if(thisType.abstractMethods().size() > 0) {
				String methods = "";
				for (Method method : thisType.concretMethods())	{
					methods += "\n" + method;
				}
				JAST.compilationUnit.reportSemanticError(line,
						"Interface must be declared concret since it defines "
								+ "the following concret methods: %s", methods);
			}
		}
		return this;
	}
